...
export function buildRecruiterHomeSteps({ hasJobs, hasQueue }: { hasJobs: boolean; hasQueue: boolean }): Step[] {
  const steps: Step[] = [];
...
export function buildCandidateHomeSteps({ hasQueue }: { hasQueue: boolean }): Step[] {
  const steps: Step[] = [
    {
...
// Sempre presentes, mesmo sem nenhuma conversa ainda — seguro rodar em qualquer estado.
