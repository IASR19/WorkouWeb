...
export const api = {
  ...
  async register(name: string, email: string, password: string, role: 'recruiter' | 'candidate', companyData?: {
    companyName?: string;
    companyCNPJ?: string;
    ...
    const response = await client.post('/auth/register', { name, email, password, role, ...companyData });
    if (response.data.accessToken) {
      localStorage.setItem('workou_token', response.data.accessToken);
      localStorage.setItem('workou_user', JSON.stringify(response.data.user));
    }
  ...
  getCurrentUser(): { id: string; name: string; email: string; role: string; availableRoles?: string[] } | null {
  ...
  async swipeCandidate(matchId: string, decision: 'approved' | 'skipped') {
    const response = await client.patch(`/matches/${matchId}/recruiter-decision`, { decision });
    return response.data;
  },

  async swipeJob(matchId: string, decision: 'approved' | 'skipped') {
    const response = await client.patch(`/matches/${matchId}/candidate-decision`, { decision });
    return response.data;
  },
  ...
  async sendMessage(conversationId: string, body: string) {
    const response = await client.post(`/conversations/${conversationId}/messages`, { body });
    return response.data;
  },
  ...
  async addSeat(dto: { name: string; email: string; password: string; cardLast4?: string }) {
  ...
  async subscribeToPlan(dto: { plan: string; cardNumber: string; cardName: string; cardExpiry: string; cardCvv: string }) {
    const response = await client.post('/companies/me/subscribe', dto);
    return response.data;
  },

  async buyExtraJob(cardLast4?: string) {
    const response = await client.post('/companies/me/extra-job', { cardLast4 });
    return response.data;
  },
